...
class TwitterClient: BDBOAuth1SessionManager {
 ...
 /*   static let sharedInstance = TwitterClient(baseURL: NSURL(string: "https://api.twitter.com")!, consumerKey: "YMrS3xdf42sVaipl7U4wk4WSY", consumerSecret: "GgqI3iv200rDBfACyr2nt1rwWMsV9hZoZbVfY3x3nBHiR5yveq")
...
*/
    ...
    func homeTimeLine(success: ([Tweet])-> (), failure: (NSError)-> ()){
        GET("1.1/statuses/home_timeline.json", parameters: nil, progress: nil, success: { (task: NSURLSessionDataTask, response: AnyObject?) -> Void in
            ...
            success(tweets)
            }, failure: {(task: NSURLSessionDataTask?, error:NSError) -> Void in failure(error)
        })
    }
